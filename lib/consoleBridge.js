export default `
<script>
  let __replit_mobile_postmessage_ready__ = false
  const __replit_mobile_message_queue__ = []
  const __replit_mobile_send_message__ = (error, parts) => {
    const message = JSON.stringify([ error, parts.join(' ') ])
    if (__replit_mobile_postmessage_ready__) {
      window.postMessage(message)
    } else {
      __replit_mobile_message_queue__.push(message)
    }
  }
  ...
  const __replit_mobile_interval__ = setInterval(() => {
    if (!!window.originalPostMessage) {
      clearInterval(__replit_mobile_interval__)
      __replit_mobile_postmessage_ready__ = true
      for (let message of __replit_mobile_message_queue__) {
        window.postMessage(message)
        document.documentElement.innerText = 'Message sent (again)!'
      }
    }
  }, 80)
</script>
